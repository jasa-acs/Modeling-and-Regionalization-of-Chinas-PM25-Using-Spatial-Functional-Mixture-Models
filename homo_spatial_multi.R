...
homo_spatial_multi2 <- function(data,cluster.num,location,fit4,fit6,P){
	x = na.approx(data)
	curve.num = dim(x)[2]
...
######### log likelihood
     loglike_list[loop] = loglike_fscm(b,y.data,e.con.g,v.con.g,con.prob,E2.gamma,lambda,Theta,phi,sigma_gamma,sigma,location,prob)  
...
fscm = list(data=data,cluster.num=cluster.num,cluster=cluster0,muhat=muhat,BIC=BIC,
sigma=sigma,sigma_gamma=sigma_gamma,phi=phi,lambda=lambda,prob=prob,loglike_list=loglike_list)
...
## log likelihood (spatial,multinomial)##
loglike_fscm = function(b,y.data,e.con.g,v.con.g,con.prob,E2.gamma,lambda,Theta,phi,sigma_gamma,sigma,station,prob) {
