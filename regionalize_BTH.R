##   Regionalization of BTH
regionalize_BTH <- function(data,cluster.num,neighbor,P){
  ...
  #    theta=theta})
     theta <-  tryCatch({ uniroot(theta_func,c(0,5))$root },error=function(e){theta=theta})
  ...
  mfcm = list(data=data,cluster.num=cluster.num,cluster=cluster0,muhat=muhat,y.neigh=y.neigh,
